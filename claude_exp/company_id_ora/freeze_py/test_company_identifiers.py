...
class test_moudulus(unittest.TestCase):
  ...
  def test_switzerland(self):  # France
    ...
    self.assertEqual(self.m_obj.validate_COMPANY_ID_bool('CH-11123456789', 'CH'), True) # Wromg chk didit not 9 but 8 is the result       
...
#        self.assertEqual(self.m_obj.validate_COMPANY_ID_bool('92345678902', 'LU') , True)   # OECD example
  ...
  def test_gg(self):
    self.assertEqual(self.m_obj.validate_COMPANY_ID_bool('123456', 'GG'), True) 
