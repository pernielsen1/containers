...
public final class RouterSession {
    ...
    private RouterSession(RouterConfig cfg, Stats stats, StopEvent stopEvent, DownstreamConnection downstream,
            CryptoClient crypto, Dispatcher dispatcher, MessageFactory<IsoMessage> factory, StopEvent reconnectEvent) {
        this.cfg = cfg;
        this.stats = stats;
    ...
    public static RouterSession connect(RouterConfig cfg, Stats stats, StopEvent stopEvent) throws IOException {
        ...
        Dispatcher dispatcher = new Dispatcher(cfg, downstream, crypto, factory, stats, reconnectEvent);

        return new RouterSession(cfg, stats, stopEvent, downstream, crypto, dispatcher, factory, reconnectEvent);
    }
    ...
    private void handleUpstream(Socket conn, SocketAddress addr, ReentrantLock writeLock) {
        ...
        try {
            while (true) {
                ...
                try {
                    req = IsoUtils.toMap(factory.parseMessage(data, 0));
                } catch (Exception e) {
    ...
    private void forward0800(Map<String, String> req) {
        ...
        byte[] frame = com.xv6.shared.ImsConnect.buildFrame(
        ...
        while (true) {
            ...
            try {
                if ("0810".equals(resp.get("t"))) {
                    ...
                    dispatcher.handleResponse(resp);
                }
            } catch (Exception e) {
