...
def is_running(name):
    """Liveness is "the actor's own /stats endpoint answers", not "we hold a live Popen handle
    for it" - a Popen-only check only ever sees actors this monitor itself launched as host
    subprocesses, and silently reports "not running" for actors started any other way (e.g. as
    part of the router_py docker-compose container's own startup command). Matches router_java's/router_cpp's
    monitor, which had to make this same call once their actors could be launched outside the
    monitor's own process tree."""
