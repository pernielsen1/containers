...
class RouterSession:
    ...
    def __init__(
        self, cfg, stats, stop_event, downstream, crypto, dispatcher, spec, reconnect_event,
        upstream_spec=None,
    ):
        self.cfg = cfg
        self.stats = stats
        ...
        self.spec = spec
        # See Dispatcher.upstream_spec - same upstream-vs-downstream leg split, defaults to
        # `spec` so router_1 (no upstream_iso_spec configured) is unaffected.
    ...
    def connect(cls, cfg, stats, stop_event, srv_sock=None) -> "RouterSession":
        ...
        upstream_spec = load_spec(cfg.upstream_iso_spec) if cfg.upstream_iso_spec else spec
        crypto = CryptoClient(
            cfg.crypto,
        ...
        return cls(
            cfg, stats, stop_event, downstream, crypto, dispatcher, spec, reconnect_event,
    ...
    def _server_upstream_loop(self, srv_sock) -> None:
    ...
    def _handle_upstream(self, conn, addr, write_lock) -> None:
        ...
        try:
            while True:
                ...
                try:
                    req, _ = iso8583.decode(data, self.upstream_spec)
                except Exception:
                ...
                try:
                    ...
                    logger.exception("unexpected error dispatching upstream message mti=%s", mti)
        finally:
            self.stats.set_connection("upstream", False)
            with self._up_ref_lock:
                if self._upstream_ref is not None and self._upstream_ref[0] is conn:
                    self._upstream_ref = None
    ...
    def _forward_0800(self, req: dict) -> None:
        ...
        try:
            ...
            logger.debug("forwarded 0800 to downstream")
        except OSError:
    ...
    def _forward_0810(self, resp: dict) -> None:
        ...
        conn, write_lock = ref
        encoded, _ = iso8583.encode(resp, self.upstream_spec)
        try:
            with write_lock:
    ...
    def _downstream_receiver(self) -> None:
        ...
        while True:
            ...
            if data[:4] == ping_marker:
                logger.debug("downstream PING pipe-cleaner received, skipping")
                continue
            ...
            try:
                ...
                continue
            self.stats.record_recv()
            logger.debug("downstream recv mti=%s", resp.get("t"))
