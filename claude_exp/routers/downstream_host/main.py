...
# downstream_shared, not shared: this component runs standalone but is also imported in-process by
# router_py's pytest suite (DownstreamHostSim) alongside router_py's own "shared" package - a
# same-named package would silently collide in sys.modules within that one process. Same pattern
# as upstream_host/main.py's upstream_shared.
from downstream_shared.command_server import CommandServer  # noqa: E402
from downstream_shared.ims_connect import PING_TRANSCODE, read_request, write_response  # noqa: E402
from downstream_shared.iso_utils import f47_decode, f47_encode, load_spec  # noqa: E402
from downstream_shared.json_log import configure_logging  # noqa: E402
from downstream_shared.ssl_utils import wrap_server_socket  # noqa: E402
from downstream_shared.stats import Stats  # noqa: E402
...
class DownstreamHostSim:
    ...
    def _dispatch_new_conn(self, conn) -> None:
        """Completes the TLS handshake (if any) and reads the first frame off-acceptor, in a
        fresh thread. The to-conn's first frame isn't sent until after the from-conn's
        resume-TPIPE, and a client that defers its TLS handshake to its first read/write
        (Java's SSLSocket does exactly this - see router_java's SslUtils.createSocket) may
        trigger the two connections' handshakes in a different order than the acceptor sees
        their TCP connects in. Either way, the acceptor must accept both TCP connections
        before blocking on either one - handshaking and reading here (not in the acceptor
        thread) avoids that deadlock."""
        if self.cfg.get("ssl_active"):
            try:
                ...
                logger.warning("TLS handshake failed for new connection", exc_info=True)
                try:
                    conn.close()
                except OSError:
                    pass
