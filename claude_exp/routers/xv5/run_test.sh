...
# directly as background processes (not via the monitor - fewer moving parts, no dependency on
# the monitor staying healthy), waits for each /stats endpoint to come up, uploads the given
# CSV, calls /start, polls /results until all rows have a response (30s deadline), then prints
# a PAN/RC/auth-code/field-47 report and the router's 30s stats.
#
# Usage:
...
# trap ... EXIT runs on every exit path, including ones triggered by a bug in this script
# itself (set -e, an unbound variable) - a plain final cleanup line would be skipped on those.
cleanup() {
  if [ "$MANUAL" -eq 0 ]; then
    for pid in "${PIDS[@]:-}"; do
...
wait_for_stats() {
  ...
  for _ in $(seq 1 30); do
    # curl -f: a non-2xx response is a nonzero curl exit, not a body for something
    # downstream to choke on - the whole point of a retry loop is tolerating "not ready
    # yet", so a miss here must not surface as anything other than "try again".
...
while true; do
  ...
  # retry - see build_router_v2.md's Glue-script safety checklist / Common pitfalls.
