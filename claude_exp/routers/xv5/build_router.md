...
`crypto_host` reads each PAN's record straight out of the loaded dict (`pan_info["pek"]`,
`pan_info["pin"]`, etc. — plain `dict.__getitem__`, not `.get()`), so a record missing one of
these keys raises `KeyError` inside `_validate()`. Flask has no try/except around the route
handler, so this becomes an unhandled 500 from `/validate_0100`/`/validate_0110`. The router's
`CryptoClient.validate()` (`router/crypto_client.py`) treats any non-2xx response as a crypto
failure and counts it toward the circuit breaker — so **one malformed PAN record can trip the
breaker and blind crypto validation for every PAN** for `crypto_breaker_cooldown_seconds`, not
just the one PAN that hit the bad record. This is a different (and worse-blast-radius) failure
mode than xv7cpp's C++ port, whose loader defaults missing fields to an empty string
(`.value(key, "")`) and so fails *silently and per-PAN* instead — see xv7cpp's `build_router.md`
pitfall of the same name for that contrast. Neither port validates `pans_defined.json`'s shape
up front; when debugging an unexpected string of declines/breaker-open events, check the file's
shape (all six keys present per PAN) before suspecting the crypto logic itself.
...
```bash
# Containerized (matches xv6java/xv7cpp):
./start.sh
...
same spawn/wait-for-stats/upload/teardown scaffolding as `run_test.sh`, but calls
...
launches each actor as `python3 ... &`, and Flask's werkzeug dev server prints its
...
**Hit while consolidating xv5/xv6java/xv7cpp into `routers/`.** The C++ port (xv7cpp) had
accreted a richer set of `LOG_DEBUG` trace points than xv5/xv6java over its own development —
one per message-handling event, not just the original handful. xv5 and xv6java were missing:
