#!/usr/bin/env python3
"""Resilience test suite - see routers/resilience.md. Port of router_py/test_resilience.py (see
router_java/test_resilience.py for the other port, which was a near-direct copy). router_cpp's
monitor.main differs from router_py's/router_java's in two ways this port has to account for -
see routers/to_do_java_cpp.md:
  1. is_running(actor) takes the actor dict, not a bare name string (router_py/router_java both
     take a name). Every call site below looks the actor up via get_actor() first to account for
     this - see is_actor_running() just below the imports.
  2. There is no standalone stop_actor(actor) helper upstream - added one to
     router_cpp/monitor/main.py (factored out of its /api/actor/<name>/stop route) to match
     router_py's/router_java's monitor.main shape, so this port can reuse it the same way.
Actor names also differ from router_py's/router_java's: router_cpp's single shared
config/router_1.json names the router "router_cpp-router" (not "router_1"), and its primary
upstream is displayed as "upstream_host" (not "upstream_1") even though it's the same shared
upstream_host component both other languages use - see ROUTER_NAME/UPSTREAM_NAME below, used
throughout instead of hardcoded router_py-style names.
...
launched via `docker exec -d` into router_cpp's already-built-and-running container (`./start.sh`)
- unlike router_py's plain host subprocesses, so the container must already be up before running
this.

Both the primary upstream and upstream_2 (and their routers) are brought up regardless of their
config.json `is_active` flag - the whole point of this suite is exercising the two-upstream
topology, not the single-upstream default used elsewhere.

Usage: python3 test_resilience.py
Output: console narration of each transition, plus routers/router_cpp/test_resilience.csv
...
# router_cpp's own actor names (see this file's module docstring, point 2, for why these aren't
# "router_1"/"upstream_1" like router_py's/router_java's).
ROUTER_NAME = "router_cpp-router"
UPSTREAM_NAME = "upstream_host"

# Default CryptoClient breaker settings (router_config.h/crypto_client.cpp) - not overridden by
# any router_1/router_2 config.json in this repo, so these constants match production behavior.
# Same values as router_py's/router_java's - all three languages default to 30s.
...
def scenario_upstream_failover(log):
    print(f"\n=== SCENARIO: upstream failover (both {UPSTREAM_NAME} and upstream_2 active) ===")
    print(
        f"    plan: start 6 actors -> settle 2s -> kill {UPSTREAM_NAME}, wait {PING_INTERVAL_S + 5}s "
        f"-> kill upstream_2, wait {PING_INTERVAL_S + 5}s -> recover both (~30s each)"
    )
    for name in ["crypto_host", "downstream_host", ROUTER_NAME, UPSTREAM_NAME, "router_2", "upstream_2"]:
        start_actor(name)
        log.log(name, "up")
    narrated_sleep(2, "letting both upstream connections settle before taking a baseline")

    before_u2 = totals("upstream_2")
    kill_actor(UPSTREAM_NAME)
    log.log(UPSTREAM_NAME, "down")
    narrated_sleep(PING_INTERVAL_S + 5, f"confirming upstream_2's 0800 ping traffic keeps going without {UPSTREAM_NAME}")
    ...
    print(f"{verdict}: upstream_2 ping traffic continued while {UPSTREAM_NAME} was down: {advanced}")
    ...
    start_actor(UPSTREAM_NAME)
    log.log(UPSTREAM_NAME, "up")
    results = send_test_csv(UPSTREAM_NAME)
    log.log(UPSTREAM_NAME, f"test.csv round-tripped {len(results)} rows: {summarize(results)}")
    print(f"{UPSTREAM_NAME} recovery: {len(results)} rows round-tripped")
...
def scenario_downstream_failure(log):
    ...
    narrated_sleep(15, f"observing {ROUTER_NAME}/router_2/{UPSTREAM_NAME}/upstream_2 with downstream_host down")
    observed = {
        ROUTER_NAME: stats(ROUTER_NAME),
        "router_2": stats("router_2"),
        UPSTREAM_NAME: stats(UPSTREAM_NAME),
    ...
    narrated_sleep(20, f"waiting for {ROUTER_NAME}/router_2 to auto-reconnect (reestablish_seconds=10 + jitter)")
    results = send_test_csv(UPSTREAM_NAME)
...
def scenario_stuck_pending_on_downstream_teardown(log):
    ...
    killed *before* sending, then the primary upstream's own already-open connection (untouched -
    no fighting over the router's single-client "server mode" slot, which is what an earlier, more
    complicated version of this scenario tripped over) is used to send test.csv. Every row gets
    genuinely stuck in the router's pending map (there's nowhere for it to go), and this checks
    that the session teardown that follows (dispatcher.cpp's drain_and_stop, once the downstream
    ...
    router = get_actor(ROUTER_NAME)

    kill_actor("downstream_host")
    log.log("downstream_host", "down (about to send a transaction with nowhere to go)")

    # send_test_csv uploads+starts test.csv on the primary upstream's existing connection and
    # polls for results for up to 15s - downstream is dead so nothing will ever complete, but
    # that gives the router's session plenty of time to notice and tear down mid-call.
    results = send_test_csv(UPSTREAM_NAME)
    log.log(UPSTREAM_NAME, f"results while downstream dead: {summarize(results)}")

    announce(f"polling {ROUTER_NAME}'s /logs for the session-teardown abandonment line", 0, 10)
    poll_start = time.time()
    ...
    log.log(ROUTER_NAME, f"session-teardown abandonment logged={abandoned_logged}")
    verdict = "PASS" if abandoned_logged else "FAIL"
    print(f"{verdict}: session teardown logged the abandoned transaction (dispatcher.cpp drain_and_stop)")

    start_actor("downstream_host")
    log.log("downstream_host", "up")
    narrated_sleep(20, f"waiting for {ROUTER_NAME} to auto-reconnect (reestablish_seconds=10 + jitter)")
    results = send_test_csv(UPSTREAM_NAME)
    log.log("downstream_host", f"recovery test.csv round-tripped {len(results)} rows: {summarize(results)}")
...
def main():
    ...
    print("Prerequisite: router_cpp's Docker container must already be running (./start.sh).")
