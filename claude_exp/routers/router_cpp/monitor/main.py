#!/usr/bin/env python3
"""router_cpp dashboard: a thin Flask app that talks to every actor's CommandServer purely over
HTTP (/stats, /stop, /log_level, /logs, plus upstream's /start//results//upload). Runs on the
host, not in the container -- the actors are only reachable at localhost because the router_cpp
...
def _load_router_2_actors():
    ...
    disabled by default (is_active: false in both config files) - mirrors router_py's existing
    router/router_2 + simulators/upstream_2 pattern. Unlike router_1, these are NOT one-shared-file
    synthesized descriptors - each gets its own config/router_2.json / config/upstream_2.json, so
    (unlike the four router_1-derived actors above, which all point at the same
    CONFIG_ABS_PATH_IN_CONTAINER) each actor here carries its own config path. router_2 reuses the
    same already-running downstream_host/crypto_host as router_1 (distinguished by client_id, see
    config/router_2.json's downstream.client_id) rather than getting its own - so no extra
    downstream/crypto actors are synthesized here."""
    actors = []
    ...
    if upstream2_path.exists():
        ...
        actors.append({
            ...
            "upstream_config_path": upstream2_path,
        })
    return actors
