#include "shared/ebcdic.h"

#include <array>

namespace shared {

namespace {

// clang-format off
constexpr std::array<uint8_t, 256> kAsciiToEbcdic = {
    ,
};

constexpr std::array<uint8_t, 256> kEbcdicToAscii = {
    ,
};
// clang-format on

constexpr uint8_t kEbcdicSpace = 0x40;

}  // namespace

std::vector<uint8_t> to_ebcdic(const std::string& s, size_t length) {
    std::vector<uint8_t> encoded;
    encoded.reserve(s.size());
    for (unsigned char c : s) {
        encoded.push_back(kAsciiToEbcdic[c]);
    }

    if (encoded.size() == length) {
        return encoded;
    }
    if (encoded.size() < length) {
        std::vector<uint8_t> padded(length - encoded.size(), kEbcdicSpace);
        padded.insert(padded.end(), encoded.begin(), encoded.end());
        return padded;
    }
    // Longer than requested: right-truncate, keeping the tail.
    return std::vector<uint8_t>(encoded.end() - static_cast<long>(length), encoded.end());
}

std::string from_ebcdic(const std::vector<uint8_t>& bytes) {
    std::string out;
    out.reserve(bytes.size());
    for (uint8_t b : bytes) {
        out.push_back(static_cast<char>(kEbcdicToAscii[b]));
    }
    return out;
}

std::vector<uint8_t> ascii_to_ebcdic_bytes(const std::vector<uint8_t>& ascii) {
    std::vector<uint8_t> out;
    out.reserve(ascii.size());
    for (uint8_t b : ascii) {
        out.push_back(kAsciiToEbcdic[b]);
    }
    return out;
}

std::vector<uint8_t> ebcdic_to_ascii_bytes(const std::vector<uint8_t>& ebcdic) {
    std::vector<uint8_t> out;
    out.reserve(ebcdic.size());
    for (uint8_t b : ebcdic) {
        out.push_back(kEbcdicToAscii[b]);
    }
    return out;
}

}  // namespace shared
