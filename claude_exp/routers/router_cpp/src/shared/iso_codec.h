...
// text, and are never translated either way. Defaults to Ascii everywhere so router_1 (and every
// existing call site) is unaffected.
enum class Encoding { Ascii, Ebcdic };

// Maps a config string ("ascii"/"ebcdic", e.g. UpstreamConfig::encoding) to the corresponding
// Encoding value. Unrecognized/empty strings default to Ascii.
inline Encoding encoding_from_config_string(const std::string& s) {
    return s == "ebcdic" ? Encoding::Ebcdic : Encoding::Ascii;
}
...
std::vector<uint8_t> build_0800(Encoding encoding = Encoding::Ascii);
std::vector<uint8_t> build_0810(const std::string& f24, Encoding encoding = Encoding::Ascii);
