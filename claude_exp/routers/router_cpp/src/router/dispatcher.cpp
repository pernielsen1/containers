...
void Dispatcher::submit_response(std::map<std::string, std::string> resp) {
...
void Dispatcher::response_worker_loop() {
    while (true) {
        std::optional<std::map<std::string, std::string>> item;
        {
            std::unique_lock<std::mutex> lock(response_queue_mutex_);
...
void Dispatcher::process(RoutedMessage& msg) {
...

    {
        ...
        pending_[router_stan] =
            PendingEntry{msg.up_fd, msg.up_write_lock, upstream_stan, std::chrono::steady_clock::now()};
