...
**Pitfall — a wrong-shaped record fails loud, but takes out every other PAN with it.**
`CryptoHostMain.validate()` reads each field straight off the parsed `Map<String, Map<String,
String>>` via `panInfo.get("pek")` etc. — a missing key yields `null`, not an exception at this
point, but `null` then flows into `CryptoUtils` helpers (`hex(null)` and friends), which throw
`NullPointerException`. The plugin route's catch-all (`catch (Exception e)`) turns that into an
HTTP 500 with `{"error": "internal error"}`. The router's `CryptoClient.validate()` treats any
`statusCode() >= 400` as a failure and counts it toward the circuit breaker — so **one
malformed PAN record can trip the breaker and blind crypto validation for every PAN** for
`cryptoBreakerCooldownSeconds`, not just the one PAN with the bad record. Same failure shape as
xv5 (see its `build_router.md`, same pitfall heading) — and a different, worse-blast-radius
failure mode than xv7cpp's C++ port, which defaults missing fields to an empty string instead of
throwing, failing silently and per-PAN rather than loudly and breaker-wide. None of the three
ports validate `pans_defined.json`'s shape up front; when debugging an unexpected string of
declines/breaker-open events, check the file's shape (all six keys present per PAN) before
suspecting the crypto logic itself.
...
the C++ port (xv7cpp), which had accreted a richer set of trace points than this port during its
own development — see the identical note under `RouterSession` below, and xv5's
...
logs the PING-skip and the received MTI. Backported from xv7cpp for parity — see the note under
`Dispatcher` above.
...
- `GET /start` — reads the CSV, resets per-run state (pending map, results, latency samples),
  launches the send loop in a daemon thread, returns `{"rows": N}`. Returns 503 if there is no
  live connection to the router yet. Accepts optional `rate`/`duration` query params — see
  "Stress testing" below.
- `GET /results` — returns the accumulated list of result maps as JSON.
- `GET /stress_stats` — returns aggregate sent/received/error counts, achieved TPS, and latency
  percentiles for the current/most recent run — see "Stress testing" below.
...
at 1,000,000), stash the original row under that STAN in a pending map (plus a send timestamp in a
parallel map, for stress-run latency tracking), build an `0100` from the row's known-field columns
plus `t=0100` and the assigned STAN, encode, `Framing.writeMessage`, `stats.recordSent()`, sleep
before the next row. Without a `duration` param this is one pass through the CSV at a fixed 20ms
sleep (legacy/functional behavior); with one, the loop instead **cycles** the rows (wrapping back
to index 0) at `1/rate` intervals until the duration elapses — see "Stress testing" below.

**Receive loop**: read frames; MTI `0810` → ignore (`continue`); MTI in `{0110, 0130, 0430}` → pop
the pending row by STAN (field 11) and also pop its send timestamp, recording the elapsed latency
into a bounded (200k-sample) list; merge every response field into the row under a `resp_`-prefixed
...
`xv6java-deploy` and gives it a `command` that builds the jar and launches all four actors
directly (`mvn -q -DskipTests package`, then `java -cp target/xv6java.jar <MainClass> --config
...
comparable container — see `../xv5/build_router.md`'s Container section.)
...
`UpstreamHostMain`'s `/start` route accepts two optional query params: `rate` (target sends/sec)
and `duration` (seconds). Omitted, `/start` behaves exactly as before — one pass through the
uploaded CSV at a fixed 20ms pace, for the functional `run_test.sh` flow. With `duration` given,
...
same shape as xv5's and xv7cpp's equivalent endpoint:
...
same `docker exec`-based spawn/wait-for-stats/upload/teardown scaffolding as `run_test.sh`, but
