#!/bin/bash
# End-to-end CLI driver (not JUnit): launches crypto_host/downstream_host/router/upstream_host as
# background `docker exec -d` processes inside the xv6java container, waits for each /stats
...
# stats. Direct Java-port analog of xv5/run_test.sh.
#
# Usage:
...
# Teardown goes through each actor's own /stop HTTP route, not a host-side PID kill: actors run
# as `docker exec -d` processes inside the container's own PID namespace, and killing the exec
# client process on the host does not reach the java process it launched (docker exec does not
# forward signals to the executed process on host-side kill). Every actor already exposes /stop
# via CommandServer, so this is both more correct here and no different in spirit from xv5's
# kill_monitor.sh, which also stops its target via an HTTP POST rather than a raw signal.
cleanup() {
  if [ "$MANUAL" -eq 0 ]; then
