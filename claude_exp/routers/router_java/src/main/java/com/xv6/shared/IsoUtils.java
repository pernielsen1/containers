...
/**
 * j8583 glue code. Port of router_py's shared/iso_utils.py, with one structural addition:
 ...
 *
 * Fields 52/55 are declared in config/test_spec.xml for parity with router_py's test_spec.json (which
...
public final class IsoUtils {
    ...
    public static MessageFactory<IsoMessage> loadFactory(String specPath) throws IOException {
        return loadFactory(specPath, null);
    }

    /** Loads a j8583 MessageFactory, optionally overriding the character encoding used for the
     * MTI, LLVAR/LLLVAR length-prefix digits, and field data (e.g. "Cp500" for an
     * EBCDIC-speaking upstream partner - see router_2/upstream_2's upstream_iso_encoding
     * config). Empirically confirmed (a standalone j8583 round-trip test, not just assumed) that
     * {@link MessageFactory#setCharacterEncoding} alone governs the MTI and ALPHA/LLVAR/LLLVAR
     * field data, but the LLVAR/LLLVAR length-prefix digits stay plain ASCII unless
     * {@link MessageFactory#setForceStringEncoding} is ALSO enabled - both are required together
     * for a fully EBCDIC wire message, matching routers/upstream_host/test_spec_ebcdic.json's
     * "len_enc": "cp500" (length digits) alongside "data_enc": "cp500" (field data). */
    public static MessageFactory<IsoMessage> loadFactory(String specPath, String characterEncoding)
            throws IOException {
        MessageFactory<IsoMessage> factory = ConfigParser.createFromUrl(new File(specPath).toURI().toURL());
        // j8583 defaults to a text/hex-encoded bitmap (16 ASCII chars), but the shared
        // routers/upstream_host Python component's pyiso8583 spec encodes the primary/secondary
        // bitmap as 8 raw binary bytes each ("data_enc": "b" in test_spec.json) - without this,
        // messages from that shared client fail to parse ("Insufficient buffer length, needs to
        // be at least 20": j8583 expects 4-byte MTI + 16-char text bitmap = 20 bytes minimum,
        // but the actual binary-bitmap message is much shorter).
        factory.setUseBinaryBitmap(true);
        if (characterEncoding != null) {
            factory.setCharacterEncoding(characterEncoding);
     ...
     * CSV columns to known fields, mirroring router_py's upstream_host.py {@code k in self.spec} check
