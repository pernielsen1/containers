#!/bin/bash
# End-to-end CLI driver (not JUnit): brings up the router_java docker-compose stack (crypto_host,
# router_main run as background processes inside one container - see docker-compose.yml's
# `command` - there is no separate long-lived dev container to `docker exec` into anymore), then
# launches downstream_host/upstream_host as bare host subprocesses (the shared
# routers/downstream_host and routers/upstream_host Python components - see
...
# router's 30s stats. Direct Java-port analog of router_py/run_test.sh and router_cpp/run_test.sh -
# same CLI surface and output format.
#
# Usage:
...
if [ "$MANUAL" -eq 0 ]; then
  if [ "$ROUTER_HOST" = "127.0.0.1" ]; then
    ...
    echo "Building and starting the router_java stack..."
    ./start.sh
...
# blocks until every worker/response-worker thread finishes its own crypto.warmup() before
# the router's upstream socket starts accepting (see Dispatcher.java) - worst case ~3.5s/thread *
# 16 threads (worker_threads=8 + response default 8) here too, since router_1.json also runs
# ssl_active mTLS end to end.
START_OK=0
