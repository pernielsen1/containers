...
A Java port of the ISO 8583 payment router built in `claude_exp/xv2` through `claude_exp/router_py`
...
dashboard was *not* reimplemented in Java; instead, router_py's Python monitor was reused as-is against
...
├── monitor/                          # ported from router_py, adapted (see "Frontend")
│   ├── main.py
│   └── static/index.html
└── src/
    ├── main/java/com/xv6/
...
the host via VSCode as normal; build/run Java via `docker exec router_java ...`. Runs with
`--network host` so the container shares the host's ports directly (important: this means
router_java's actor ports, 8080-8083, are the *same* ports router_py's Python actors use — the two can't
...
container): launches all four actors via `docker exec -d router_java java -cp target/router_java.jar
...
## Frontend: router_py's Python monitor, reused as-is against the Java backend
...
router_py's monitor works against router_java's actors with only the launch/liveness-tracking layer
adapted — the frontend HTML/JS is untouched.

**Runs on the host**, not inside the router_java container — it's a thin dashboard process, not part
of the Java build.

Adaptations in `monitor/main.py` (ported from `router_py/monitor/main.py`):
- `MAIN_CLASS_BY_TYPE` (fully-qualified Java class names) replaces `SCRIPTS_BY_TYPE`.
- `launch_actor()` runs `docker exec -d router_java java -cp target/router_java.jar <MainClass>
...
- `/api/csv_files` reads each upstream actor's own `input_dir` field from its config instead of
  assuming a fixed `"input"` subfolder name (router_java's upstream configs name it e.g.
  `upstream_1_input`).

`monitor_start.sh` / `monitor_stop.sh` (new, at the project root): pidfile-based lifecycle,
mirroring router_py's `run/monitor.sh` / `kill_monitor.sh` — deliberately *not* `pgrep -f
"monitor/main.py"`, since that exact pattern already left a zombie router_py monitor alive on port 8090
...
browser bridged over a WebSocket to a PTY running `docker exec -it router_java bash` on the monitor
...
- **Header "Attach Shell" button** — `GET /api/commands` returns `{"shell": "docker exec -it
  router_java bash"}` (the same thing `terminal.sh` runs); the button copies it to the clipboard.
- **Per-actor "Commands" button** (next to "Logs") opens a modal with two commands, each with its
  ...
  - **Force-kill** — an escape hatch for when the graceful `/stop` HTTP call doesn't respond.
    ...
    look up the PID via `docker exec router_java jps -lm | grep -F "$PATTERN" | cut -d" " -f1` (only
    `jps` itself needs `docker exec`; `grep`/`cut` run in the operator's own shell, which is what
    keeps this free of the nested-quoting a one-liner would need), then `docker exec router_java kill
  ...
  - **Tail live console** — `docker exec router_java tail -F logs/<name>.console.log`. Needed a small
...
- router_py (Python) and router_java (Java) actors use the *same* ports (8080-8083) and the monitor uses
