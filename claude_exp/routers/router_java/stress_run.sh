#!/bin/bash
# Stress-test CLI driver: brings up the router_java docker-compose stack via ./start.sh
...
# to stderr. Direct Java-port analog of router_py/stress_run.sh and router_cpp/stress_run.sh.
#
...
# just RouterMain's crypto client at it; the container's own local crypto_host stub still starts
...
# via their own /stop routes. Every run gets a fresh container: unlike the old persistent-dev-
# container setup, there's no "leave it running for an interactive session" case to preserve
# anymore, so ./stop.sh runs unconditionally, same as router_cpp.
cleanup() {
  if [ "$MANUAL" -eq 0 ]; then
...
if [ "$MANUAL" -eq 0 ]; then
  if [ "$ROUTER_HOST" = "127.0.0.1" ]; then
    ...
    echo "Building and starting the router_java stack (perf config)..." >&2
...
# Dispatcher.start() blocks until every worker/response-worker thread has completed its own
