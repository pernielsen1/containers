...
Port the ISO 8583 payment router built in `claude_exp/xv2` through `claude_exp/router_py` (all Python)
...
- **Scope**: port the router + simulators (crypto_host, downstream_host, upstream_host) — not
  ...
  proven, mirroring how the Python lineage (xv2→router_py) itself grew incrementally.
...
| Concept | Python (xv2-router_py) | Java (router_java) | Future C++ |
|---|---|---|---|
...
`claude_exp/router_py/` is the reference Python implementation to port field-for-field: `router/*.py`,
