#!/bin/bash
# End-to-end CLI driver (not Catch2): brings up the xv7cpp docker-compose stack (all four actors
# run as background processes inside one container - see docker-compose.yml's `command` - there is
# no separate long-lived dev container to `docker exec` into, unlike xv6java's), waits for each
# /stats endpoint to come up, uploads the given CSV to upstream_host, calls /start, polls /results
# until all rows have a response (30s deadline), then prints a PAN/RC/auth-code/field-47 report and
# the router's 30s stats. Direct C++-port analog of xv5/run_test.sh and xv6java/run_test.sh -
# same CLI surface and output format, adapted spawn/teardown to this port's single-container model.
#
# Usage:
...
if [ "$MANUAL" -eq 0 ]; then
  echo "Building and starting the xv7cpp stack..."
  ./start.sh
else
