...
(`../xv6java/build_router.md`). Behavior — wire formats, message flows, config semantics, and the
...
The Python (xv5) and Java (xv6java) ports have the same missing-field gap in their loaders, but a
worse blast radius: both index straight into the parsed record (`dict[key]` / `Map.get(key)`)
rather than defaulting, so a missing field throws (`KeyError` / `NullPointerException`) instead of
silently becoming `""`. Their crypto_host has no catch around the validation route, so this
surfaces as an HTTP 500, which their router's crypto client circuit breaker counts as a failure —
enough consecutive malformed-PAN validations trip the breaker and blind crypto validation for
*every* PAN for the cooldown period, not just the one with the bad record. See the identically-named
pitfall in xv5's and xv6java's `build_router.md` for the details.
...
- `GET /start` — reads the CSV, resets per-run state (pending map, results, latency samples),
  launches the send loop in a detached thread, returns `{"rows": N}`. Returns `503` if there is no
  live connection to the router yet. Accepts optional `rate`/`duration` query params (via
  `req.has_param`/`req.get_param_value`) — see "Stress testing" below.
...
at 1,000,000), stash the original row under that STAN in a pending map (plus a
`steady_clock::time_point` in a parallel map, for stress-run latency tracking), build an `0100`
from the row's known-field columns plus `t=0100` and the assigned STAN, encode,
`framing::write_message`, `stats.record_sent()`, sleep before the next row. Without a `duration`
param this is one pass through the CSV at a fixed 20ms sleep (legacy/functional behavior); with
one, the loop instead **cycles** the rows (wrapping back to index 0) at `1/rate` intervals until
the duration elapses — see "Stress testing" below.

**Receive loop**: read frames; MTI `0810` → ignore; MTI in `{0110, 0130, 0430}` → pop the pending row
by STAN (field 11) and also its send timestamp, recording the elapsed latency into a bounded
(200k-sample) list; merge every response field into the row under a `resp_`-prefixed key (e.g.
...
section below for the CLI-compatibility note versus xv5's/xv6java's `run_test.sh`. The manual/CLI
...
same shape as xv5's and xv6java's equivalent endpoint:
...
docker-compose build/up output — see the pitfall note in xv5's doc, which applies here too:
...
`xv5/run_test.sh` and `xv6java/run_test.sh` (same `[--manual] <csv_file>` usage, same results-table
and 30s-stats output), but its spawn/teardown wraps `./start.sh`/`./stop.sh` rather than
launching per-actor processes: unlike xv6java, there is no separate long-lived dev container to
...
row counting/results polling/report formatting are copied verbatim from xv5's script.
