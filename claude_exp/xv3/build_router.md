...
**Traffic counters**: `set_connection`/`set_gauge` are not the only `Stats` calls the router
needs to make. `stats.record_sent()`/`stats.record_recv()` must also be called at every actual
wire I/O point (decode of an upstream frame, decode of a downstream frame, write to upstream,
write to downstream) — both in the dispatcher (`_process` after `downstream.send()`,
...
6. Encode and write to `entry.up_conn` under `entry.up_write_lock`, then `stats.record_sent()` —
   ...
   session teardown closing the upstream socket from a different thread can race it (the same
   class of race `_recv_exact` normalizes on the read side, but a write's `OSError` — e.g.
   EBADF — has no such central wrapping, so each write site guards itself and logs+drops on
   failure rather than crashing the thread)
...
- MTI routing:
  ...
  - `0800` → `_forward_0800(...)` (re-encode, wrap in IMS frame, send to downstream,
    `stats.record_sent()` on success — wrapped in `try/except OSError` since this write can
    race a concurrent teardown closing the downstream socket from another thread)
  - other → log warning
- On `ConnectionError`: sets `reconnect_event`. Catch `ConnectionError` specifically, not a
  bare `OSError` — `shared/framing.py`'s `_recv_exact` normalizes *any* `OSError` from
  `sock.recv()` (including a local close racing this blocked read, e.g. EBADF) into
  `ConnectionError`, so every reader built on `read_message`/`read_request` only ever needs to
  catch the one exception type for both a remote disconnect and a local teardown race.
...
- Skips frames whose first 4 bytes == `"PING".encode("cp500")` — both halves of the EBCDIC
  marker, see `downstream_host`'s `_route_frame` above; an ASCII `"PING"` from the sender side
  will not be recognized here and falls through to a `DecodeError`
- Decodes ISO 8583 → `stats.record_recv()`
- MTI `0810` → `_forward_0810(resp)` (re-encode, write to upstream via `_upstream_ref`,
  `stats.record_sent()` on success, same `try/except OSError` write-race guard as above)
- Other → `dispatcher.handle_response(resp)`
- `ConnectionError` → `stats.set_connection("downstream", False)`, sets `reconnect_event`
  (same normalization via `_recv_exact` as `_handle_upstream`, above)
...
- `_route_frame`:
  - `PING_TRANSCODE` → send `"PING".encode("cp500") + "PIPES cleaned".encode("cp500")` to
    ...
    An ASCII `"PING"` prefix here will not be recognized as a ping by the router; the bytes
    fall through to `iso8583.decode()` and raise a `DecodeError`.
  ...
  - `0100` → `_process_0100(req, pans)`:
    ...
    - **Every response (approve or decline) must echo `f47` back**, not just field 39: take
      the request's decoded `f47_decode(req.get("47", ""))`, set `message_type="0110"` and
      `response_code` to the rc just decided, and put `f47_encode(...)` of that into the
      response dict's `"47"` key. This is required even though nothing above this point reads
      `f47` back out — `crypto_host`'s ARPC step (`calculate_arpc_method1`, see below) only
      runs on `validate_0110`, and the only place the original `f55` cryptogram/ATC can reach
      that call is by `downstream_host` round-tripping the request's `f47` into its response.
      Skipping this means ARPC silently never gets computed; nothing in the router or
      crypto_host raises an error to catch the omission.
...
**`POST /upload` overwrites `input_dir/test_cases.csv` in place.** That means a given upstream
instance's seed CSV (e.g. `simulators/upstream_1/input/test_cases.csv`) is mutable test state,
not an immutable fixture — any manual run, monitor-driven upload, or `upload_path` call against
that instance permanently replaces its content with whatever was last uploaded. A test that
asserts against the *specific rows* of an instance's default CSV (rather than just its row
count) should write its own known content into that path at setup time instead of trusting
...
`SIGTERM` (only on normal interpreter exit, e.g. `sys.exit()` or falling off `main()`), so without
the explicit signal handler, `kill <monitor_pid>` — or `docker stop`/`systemd stop` against the
monitor in a real deployment — orphans every actor subprocess it spawned, silently leaking
processes and squatting on their ports for any future restart. Verified by: starting the monitor,
spawning actors via `/api/start_all`, sending a raw `kill <pid>` (not `POST /stop`), and confirming
via `ps` that all spawned children exit too.
...
`from router...` import) — running `python3 router/main.py` or
`python3 simulators/crypto_host/main.py` directly from the project root, exactly as shown
above, otherwise fails with `ModuleNotFoundError: No module named 'shared'` (or `'router'`).
This is because Python only adds the *script's own directory* to `sys.path`, not the cwd, when
invoked as `python3 path/to/script.py` — `monitor/main.py` is the one exception that needs no
bootstrap, since it never imports `shared` or `router`. The bootstrap line differs by nesting
depth: two `os.path.dirname()` calls for `router/main.py`, three for `simulators/*/main.py`.
...
- `run/monitor.sh` — starts the monitor, first writing its own PID to `run/.monitor.pid`
  (`echo $$ > run/.monitor.pid` before `exec`— `exec` replaces the script's process image but
  keeps the same PID, so the file stays valid for the lifetime of the monitor)
- `run/kill_monitor.sh` — `POST /stop` to the monitor, then polls the PID read from
  `run/.monitor.pid` for up to 30s, then sends `SIGKILL` if it hasn't exited (covers a monitor
  that's wedged or has a stuck subprocess), then removes the pidfile. Don't use `pgrep -f
  "monitor/main.py"` to find the PID instead — it pattern-matches the full command line of
  every process on the system, including unrelated processes that merely mention that path
  (e.g. a shell that's currently running a command containing that string), and can kill the
  wrong thing.
...
- **`test_csv_files/test_crypt.csv` is not wired up to anything in this spec.** Its columns
  (`f2;message_type;f14;f52;f55;cvv2;aav;expected_rc`) don't match the upstream CSV format
  documented above (`2;3;4;11;expected_39`, field-number-keyed) — it looks like a fixture for a
  more direct crypto_host test (feeding `f52`/`f55`/`cvv2`/`aav` straight to `_validate` without
  going through a full upstream→router→downstream round trip) that was never built. Leave it
  alone until that test exists; don't try to make `upstream_host`'s CSV parser understand it.
