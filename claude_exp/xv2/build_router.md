...
class CommandServer:
    ...
    def register(self, path, methods=("GET",)) -> decorator   # add custom routes
    def start(self)                                            # runs Flask in daemon thread
```

Built-in routes:
- `GET /stats` → `stats.snapshot()` as JSON
- `GET|POST /stop` → sets `stop_event`, returns `{"status":"stopping"}`
- `GET|POST /log_level` → GET returns current level; POST `{"level":"DEBUG"}` sets it
- `GET /logs` → JSON array of log lines; `?format=text` returns plain text

LogBuffer is installed on the root logger inside `__init__`.
...
class CryptoClient:
    ...
    def validate(self, endpoint: str, pan: str, f47: str) -> str:
        # POST {base}/{endpoint} with JSON {"f2": pan, "f47": f47}
        # Returns response["f47"], or original f47 on any error
...
class Dispatcher:
    ...
    def start(self)              # spawns cfg.worker_threads daemon threads
    def submit(self, msg: RoutedMessage)   # non-blocking enqueue
    def handle_response(self, resp: dict)  # called from ds-receiver thread
    def drain_and_stop(self)     # sends None sentinels, joins workers
```

**STAN rewriting** — each router maintains its own counter (6-digit, wraps at 1,000,000):
- `_next_stan()` → zero-padded string e.g. `"000042"`
- On 0100: save `(upstream_conn, upstream_lock, upstream_stan)` keyed by `router_stan`; send to downstream with `router_stan` in field 11
...
def run(cfg=None, stop_event=None, stats=None, _config_base=None):
    # 1. Create Stats, CommandServer (start it)
    # 2. If upstream.mode == "server": create UpstreamServer (lives outside session loop)
    # 3. Main loop (while not stop_event):
    #    a. RouterSession.connect(...) — catch OSError → wait reestablish_seconds, continue
    #    b. session.run_until_disconnect(srv_sock)
    #    c. If not stop_event: wait reestablish_seconds
...
**Per-actor card (full size):** status dot, connection dots (router only: upstream + downstream), 30s/60s and total sent/recv counters, last-recv time, log-level selector, Logs / Start / Stop buttons.
...
| Test file | Covers |
|---|---|
| `test_framing.py` | length-prefix framing round-trip (all 4 `length_field_type` encodings) |
