...
class PnCrypto(): 
    ...
    def do_RSA(self, operation, key, data, mode="OAEP", hash="SHA256"):
        ...
        der = base64.b64decode(self.clean_pem(key))
        if (mode == "OAEP"):
            pad_func = padding.OAEP(
                mgf=padding.MGF1(algorithm=hashes.SHA256()),
                algorithm=hashes.SHA256(),
                label=None
            )
        if (mode == "PKCS1_v1_5"):
            pad_func = padding.PKCS1v15()

        data_bytes = bytes.fromhex(data)
        if (operation == "encrypt"):
            public_rsa_key =  serialization.load_der_public_key(der, None)
            return public_rsa_key.encrypt(data_bytes, pad_func).hex()
        if (operation == "decrypt"):
            private_rsa_key = serialization.load_der_private_key(
                der,
                password=None,
            )
